...
```{r}
random_forest_model <- randomForest(x = x_train, y = y_train$targetValues)
predictions <-  predict(random_forest_model, x_test)
table(y_test$targetValues, predictions)
```

```{r}
random_forest_model2 <- randomForest(x = x_train, y =  y_train$targetValues,
                        xtest = x_test, ytest = y_test$targetValues)
