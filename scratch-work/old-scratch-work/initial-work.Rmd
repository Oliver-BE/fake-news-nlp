...
https://stackoverflow.com/questions/33193152/unable-to-convert-a-corpus-to-data-frame-in-r for corpus to df

```{r} 
# this is our input
trump <- "'Trump approval rating    better than Obama  and Reagan at same 
            point in their presidencies.'" 
# convert the input to a vector corpus using the `tm` package
trump_corpus <- VCorpus(VectorSource(trump))
# examine the original content
content(trump_corpus[[1]])

# remove any punctuation from the corpus
trump_corpus <- tm_map(trump_corpus, removePunctuation)
...
trump_corpus <- tm_map(trump_corpus, removeWords, all_stops)
# strip any out of place white space from the corpus
trump_corpus <- tm_map(trump_corpus, stripWhitespace)
# re-examine our cleaned corpus
content(trump_corpus[[1]])

# now, we can go ahead and lemmatize the words in the corpus 
trump_corpus <- tm_map(trump_corpus, lemmatize_words)
# trump_corpus <- tm_map(trump_corpus, PlainTextDocument)

# examine our final cleaned corpus
content(trump_corpus[[1]])
```
